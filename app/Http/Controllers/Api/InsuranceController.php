        ...
        foreach ($records as $insurance) {
            ...
            elseif ((int) $insurance->insurance_type_id === 12) {
                ...
                if (
                    ...
                    $threshold = $insurance->benchmark;

                    // Step 3: Calculate PPI
                    $ppi = ($sub->real_time_market_price / $bep) * 100;

                    // Step 4: Calculate actual trigger price in rupees
                    $triggerPrice = ($threshold / 100) * $bep;

                    // Step 5: Compensation Logic
                    $comp = 0;
                    $lossStatus = 'no loss';

                    // ✅ Your actual logic
                    if ($ppi < $threshold) {
                        // PPI is less → now check second condition
                        if ($sub->real_time_market_price < $triggerPrice) {
                            $comp = $sub->ensured_yield * ($triggerPrice - $sub->real_time_market_price);
                            $lossStatus = 'loss';
                        }
                    }

                    // Step 6: Info to return
                    $compInfo = [
            ...
            elseif ((int) $insurance->insurance_type_id === 8) {
                ...
                $weatherData = \App\Models\VillageWeatherDailySummary::where('village_id', $villageId)
                    ->whereBetween('date', [now()->subDays(14)->toDateString(), now()->toDateString()])
                    ->get();

                $rainfallSum = $weatherData->sum('avg_rainfall');
                $temperatureAvg = $weatherData->avg('avg_temperature');

                $compInfo = [
                    'type' => 'Weather Index',
                    'admin_defined_avg_temperature' => round($adminTemp, 2),
                    'admin_defined_avg_rainfall' => round($adminRain, 2),
                    'your_14_day_avg_temperature' => round($temperatureAvg, 2),
                    'your_14_day_total_rainfall' => round($rainfallSum, 2),
                    'compensation' => round($comp, 2),
